...
resolver compiles selected typed Test definitions against the in-memory
candidate and asks the driver to execute the ordinary native validation
harness. A failed or unavailable required test run leaves the previous manifest
authoritative and publishes no evidence for the rejected candidate.
...
the complete program and its selected tests still succeed. Development-only
resolution may add `--allow-host-toolchain`; release resolution supplies the
pinned roots below. Packages without selected tests need no native runner.
