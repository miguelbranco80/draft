# Bootstrap README implementation snapshot

Status: historical source snapshot retained during the documentation reorganization.

The following section is the complete former root README implementation narrative.
Its information is now organized across the implementation, operations, target,
and release documentation. This copy remains as a no-loss audit source and
records the wording and command examples of the completed bootstrap period.

## Bootstrap compiler

The bootstrap compiler is being implemented in a deliberately direct C++20
subset under the rules in [AGENTS.md](../../AGENTS.md) and the sequencing preserved in the
[historical first implementation plan](first-implementation-plan.md). The checked-in implementation
...
dependencies because they may introduce any missing package name. The checked
`examples/agent-acceptance` fixture exercises this boundary through real Codex:
its generated member rebuilds a checked struct that is consumed by both the
native executable and its separately compiled test graph.
