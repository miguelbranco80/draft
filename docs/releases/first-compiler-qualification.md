...
in [the AArch64 macOS toolchain document](aarch64-macos-toolchain.md).

That result is the locked `draft-core-bootstrap-v1` qualification baseline.
The current `draft-core-bootstrap-v2` source matrix contains 21 programs after
adding the language-tour, console, file-I/O, and denial examples. All 21 pass
the ordinary and sanitizer-hosted native conformance gates; the four additions
still require a selected-root locked rerun before a v2 release can claim the
