...
| --- | --- |
| [`language-tour`](language-tour/) | A readable ordinary-language tour: constants, `when`, named/default arguments, arrays, slices, string-byte and tuple-pattern iteration, structs, enums, variant `Result` and `Option`, tuples, distinct types, parametric types and procedures, pointers with automatic member dereference, `for`, `switch`, `defer`, assertions, `docs`, and console output. |
...
| [`turbo-editor`](turbo-editor/) | A useful full-screen terminal editor built from the reusable Turbo packages: real File/Edit/Build/Window/Help menus, Sources/open-buffer navigation, multiple exact-byte buffers, Unicode grapheme movement, Find/Replace/Go-to-line, shortcut help, mouse selection, bounded undo/redo, disk-conflict detection, and managed tool windows with Tile/Cascade arrangement. |
...
| [`runtime-traps`](runtime-traps/) | Runtime-selected mandatory trap paths for assertions, division, shifts, conversions, Unicode and enum checks, and bounds checks. The native test runner expects each selector to trap. |
