...
| [`turbo-ui-gallery`](turbo-ui-gallery/) | The independent visual proof for `lib/turbo_ui`: classic blue menus, buttons, checkboxes, lists, scrollbars, splitters, dialogs, overlapping draggable/resizable windows, mouse input, keyboard focus, resize handling, and differential rendering. |
| [`turbo-editor`](turbo-editor/) | A useful full-screen terminal editor built from the reusable Turbo packages: multiple file buffers, exact-byte open/save, Unicode grapheme movement, mouse selection, literal search, bounded undo/redo, dirty quit protection, disk-conflict detection, and Files/Buffers windows. |
