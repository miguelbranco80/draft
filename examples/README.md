...
| [`turbo-ui-gallery`](turbo-ui-gallery/) | The independent visual proof for `lib/turbo_ui`: classic drop-down menus, buttons, check/radio controls, UTF-8 text and combo boxes, lists, splitters, true modal dialogs, close/zoom/resize window chrome, mouse and keyboard input, and differential rendering. |
| [`turbo-editor`](turbo-editor/) | A useful full-screen terminal editor built from the reusable Turbo packages: real File/Edit/Project/Window menus, multiple file buffers, exact-byte open/save, Unicode grapheme movement, reusable search and dirty-quit dialogs, mouse selection, bounded undo/redo, disk-conflict detection, and managed tool windows. |
