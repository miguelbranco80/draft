...
| Initial core package set from specification section 7 | `core/runtime`, `c`, `option`, `result`, `memory`, `heap`, `array`, `map`, `io`, `testing`, `benchmark`, and `time`; container examples check, lower, and execute natively | Incomplete | `os`, `atomic`, and `thread` are absent; memory arenas/owned buffers/strings/virtual memory and full test/benchmark runners are also absent. |
...
The next implementation slice completes OS/process exposure, atomics, threads,
and the remaining Draft 1 runtime, assembly, and C ABI
