...
| Complete handwritten language through HIR, MIR, LLVM, and native execution | `src/sema/body_checker.*`, `src/mir`, `src/backend`, native examples | Partially implemented | Complete foreign aggregate ABI coverage, runtime/core surface, and the release-native conformance matrix. |
...
The next implementation slice completes the remaining Draft 1 C ABI and native
artifact surfaces needed by representative programs. The validation and evidence
harness then needs to exercise those complete programs through the locked native
seam rather than relying only on unit-level compiler proofs.
