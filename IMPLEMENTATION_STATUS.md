...
| Dependency-ordered synthesis and opaque interface completeness sets | staged resolver/compiler passes and resolver tests, including symmetric captured-request proof that same-package sites cannot observe either generated name; early expression/statement sites reached by constants and declaration-level `when`; direct synthesized `when` conditions; and a real provider-free AArch64 executable that combines declaration, member, expression, and statement expansions, rebuilds byte-identically, and exits successfully | Implemented for package and first same-package compile-time dependency rounds | Extend the same scheduling proof to synthesis reached only through type/layout integer recipes and to finer dependent completeness sets among otherwise independent types. The final locked replay still requires the pinned release toolchain/SDK. |
