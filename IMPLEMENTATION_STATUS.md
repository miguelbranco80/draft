...
| Provider-independent docs, judgments, and synthesis obligations | agent metadata/obligation modules, canonical expected/binding type spellings, ordered parametric constraints, explicit target/assembly context, persistent source coordinates, comment-free canonical enclosing declaration context, lexically active denial selectors, package/enclosing-declaration docs with exact attachments, and tests | Implemented foundation | Context still lacks the full bounded semantic dependency closure, denial-resolved usable-symbol filtering, a reduced semantic skeleton distinct from enclosing token context, tests, benchmarks, and dominating judgment claims. |
