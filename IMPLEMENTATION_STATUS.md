...
| --- | --- | --- | --- |
| Small, direct C++20 bootstrap with warnings, sanitizers, and tests | `AGENTS.md`, root `CMakeLists.txt`, the compiler/test targets, and a passing 51-test AddressSanitizer/UndefinedBehaviorSanitizer run | Implemented foundation | Repeat the release matrix with both supported host compilers available to release engineering. |
| One explicit AArch64 macOS target profile | `src/target/profile.*`, `draft_target_profile_tests`, and the locked `examples/agent-acceptance` executable built with exact LLVM/LLD 22.1.8 and macOS 26.5 SDK content trees | Implemented and qualification-tested for the first target | Repeat the complete native matrix with the selected self-contained release distribution; the acceptance root was assembled for qualification. |
...
| Ordinary offline builds consume pins without a provider | staged offline compiler path, resolver tests, locked executable/archive adapter tests, byte-identical repeated real builds of all five output kinds, and provider-free reuse of all three real Codex pins in `examples/agent-acceptance` | Implemented for the first host gate and locked acceptance executable | Repeat the full five-kind proof with the selected release toolchain/SDK. |
| `draft build --locked` with no ambient external search | Versioned external-input rows, resolved-program binding, content-tree verification, canonical existing-package/workspace roots before no-follow store access, explicit toolchain/SDK/provider/summary/runtime-asset CLI roots, clean process environment, absolute Clang/linker/archiver/dsymutil paths, provider snapshots, complete relocated file-or-directory runtime-asset verification, consumed summary verification, SDK/link flags, optional exact-key test/benchmark evidence gates, exact per-site active judgment-evidence verification with no provider call, and a real acceptance build requiring both selected evidence kinds under pinned LLVM/LLD 22.1.8 and SDK trees | Implemented for every external role consumed by the first native adapter; locked acceptance passed | Runtime assets are returned to embedding deployment tooling rather than assigned an unspecified CLI output layout; unsupported future external roles still fail closed. Repeat with the selected release distribution, and add a trustworthy content-addressed snapshot/cache so cold complete-SDK re-verification is not needlessly expensive. |
...
v125, and is accepted consistently by `build`, `resolve`, and `judge`. Test and
...
audit, then closes the remaining release gates: a selected self-contained
toolchain/Codex distribution, the full pinned-native matrix, versioned validation
instrumentation, and a trustworthy cache for cold content-tree verification.
