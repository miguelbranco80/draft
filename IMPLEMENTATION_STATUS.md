...
| Provider-independent docs, judgments, and synthesis obligations | agent metadata/obligation modules, canonical expected/binding type spellings plus complete referenced type graphs/layouts/members, compact visible imported-package interfaces with constants and effect contracts, ordered parametric constraints, explicit target/assembly context, persistent source coordinates, comment-free canonical enclosing declaration context, lexically active denial selectors, semantic-identity filtering of denied locals/declarations/import members/packages, denial-filtered typed runtime `Context` fields even for early interface synthesis, package/enclosing-declaration docs and positionally guiding judgment claims with exact attachments, and tests | Implemented foundation | Context still lacks on-demand private/local declaration dependency closure, a reduced semantic skeleton distinct from enclosing token context, imported documentation bytes, relevant tests and benchmarks, and typed branch refinements for judgment context. |
