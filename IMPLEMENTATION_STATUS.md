...
| Codex adapter behind a provider-neutral boundary | `src/elaborator/codex_cli.*`, fixed hashed timeout/retry policy, user/SIGINT cancellation with forced child kill-and-reap, and adapter tests | Implemented first adapter | Pin the complete executable distribution identity, not merely the selected launcher bytes. |
