...
| Symbols, scopes, types, constants, layouts, parametrics, and `when` | `src/sema` and semantic/type/constant/interface tests; declaration-modifier boundary tests; aliases inherit closed generic-constraint membership while `distinct` wrappers retain numeric, logical, and comparison operators without joining closed generic constraints; target-owned SIMD shape validation; enum zero-value and explicit tagged-union discriminator-capacity validation; canonical typed dependent integer expressions retain arithmetic over multiple value parameters through local specialization, explicit procedure applications, nested nominal composition, structural alias composition, package-interface ordinals, full procedure-dependent nominal/structural value arguments, and transitive owner-evaluated layout requests across three packages | Broadly implemented | Add algebraic inference for nontrivial dependent patterns where a unique checked solution exists; audit every Draft 1 operator/type combination and complete remaining C-layout edge-case coverage. |
