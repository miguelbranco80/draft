...
| --- | --- | --- | --- |
| Small, direct C++20 bootstrap with warnings, sanitizers, and tests | `AGENTS.md`, root `CMakeLists.txt`, the compiler/test targets, and a passing 39-test AddressSanitizer/UndefinedBehaviorSanitizer run | Implemented foundation | Repeat the release matrix with both supported host compilers available to release engineering. |
...
| Crash-safe and deterministic release verification | Atomic pin-store tests, interrupted object-before-manifest recovery, deterministic serializers, byte-identical repeated real builds of all five artifact kinds, and passing ordinary/sanitized 39-test suites | Implemented foundation | Expand fault injection and malformed-store fuzzing, then run the same gates through the pinned LLVM 22.1/SDK distribution. |
