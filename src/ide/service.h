...
// CommentExpansionResult belongs only to the ephemeral selected `//?` or `//!`
// editor command. It has no semantic diagnostic/span fields because expansion
// does not compile or replace those session products. source_length is the
// complete active-file replacement available through its copy operation after
// success.
typedef struct DraftCompilerServiceCommentExpansionResult {
  uint8_t success;
  size_t source_length;
  uint64_t elapsed_nanoseconds;
} DraftCompilerServiceCommentExpansionResult;
...
// expand_comment borrows the active source plus every dirty reachable overlay,
// the active row, the exact half-open byte range of its selected contiguous
// `//?` or `//!` lines, and their concatenated text. It invokes the default
// Codex editor policy synchronously but never compiles, saves, diffs, or mutates
// source. The complete replacement file and failure text remain session-owned
// until the next expansion call and use the ordinary complete-size copy
// contract.
DRAFT_COMPILER_SERVICE_API void draft_compiler_session_expand_comment(
    void *session,
    const DraftCompilerServiceOverlay *overlays,
    size_t overlay_count,
    size_t active_overlay,
    size_t prompt_start,
    size_t prompt_end,
