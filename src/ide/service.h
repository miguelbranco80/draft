...
// CommentExpansionResult belongs only to the ephemeral `//?` editor command.
// It has no semantic diagnostic/span fields because expansion does not compile
// or replace those session products. Each offset addresses the active overlay;
// each length is the exact generated byte count available through its matching
// copy operation on success. Empty slots have zero length but retain their legal
// insertion offsets.
...
// the active row, the exact half-open byte range of its contiguous `//?` lines,
// and their concatenated text. It invokes the default Codex editor policy
// synchronously but never compiles, saves, or mutates source. Generated slot
// bytes and failure text remain session-owned until the next expansion call and
// use the ordinary complete-size copy contract.
...
DRAFT_COMPILER_SERVICE_API size_t
draft_compiler_session_copy_comment_expansion_local(
    void *session, uint8_t *destination, size_t capacity);
