// Shared content-addressed evidence store implementation.
...
void store_error(DiagnosticSink &diagnostics, std::string message) {
  diagnostics.error(
      SourceRange::invalid(), "evidence store: " + std::move(message));
}
...
[[nodiscard]] bool synchronize_path(
    ...
    DiagnosticSink &diagnostics) {
  store_error(diagnostics, "durable writes are not implemented on this host");
  return false;
}
...
public:
  StoreLock(const std::filesystem::path &, DiagnosticSink &diagnostics) {
    store_error(diagnostics, "locking is not implemented on this host");
  }
...
[[nodiscard]] bool synchronize_path(
  ...
  if (descriptor < 0) {
    store_error(
        diagnostics,
        "cannot open path for synchronization: '" + path.string() + "'");
    return false;
  }
  ...
  if (!ok) {
    store_error(
        diagnostics, "cannot synchronize path: '" + path.string() + "'");
  }
  return ok;
}
...
[[nodiscard]] bool write_atomic(
  ...
  if (!output) {
    store_error(
        diagnostics, "cannot open temporary file '" + temporary.string() + "'");
    return false;
  }
  ...
  if (!output) {
    store_error(
        diagnostics, "cannot write temporary file '" + temporary.string() + "'");
...
[[nodiscard]] std::string serialize_state(const StoredState &state) {
  std::string result = "draft-evidence-state-v1\nkey " + state.key.hex() +
      "\nactive ";
...
[[nodiscard]] bool parse_state(
  ...
  if (lines.size() < 4 || lines[0] != "draft-evidence-state-v1") {
  ...
  if (parsed.attempts.empty()) {
    store_error(diagnostics, "state file contains no evidence attempt");
    return false;
  }
...
[[nodiscard]] bool read_evidence_object(
  ...
  if (codec.decode == nullptr ||
      !codec.decode(codec.state, bytes, key, ordinal, passed, diagnostics)) {
    return false;
  }
  return true;
}
...
[[nodiscard]] bool load_state_unlocked(
  ...
  if (stored.active.has_value()) {
    if (*stored.active != stored.attempts.back() || !last_passed) {
      store_error(
          diagnostics, "active evidence is not the latest passing attempt");
      return false;
    }
    state.status = EvidenceAttemptStateStatus::Active;
    state.active_digest = stored.active;
  } else {
    if (last_passed) {
      store_error(
          diagnostics, "latest passing attempt is incorrectly revoked");
      return false;
    }
    state.status = EvidenceAttemptStateStatus::Revoked;
  }
...
bool load_evidence_attempt_state(
    const std::filesystem::path &workspace_directory,
    const Sha256Digest &key,
    EvidenceAttemptCodec codec,
    EvidenceAttemptState &state,
    DiagnosticSink &diagnostics) {
  if (codec.decode == nullptr) {
    store_error(diagnostics, "typed evidence decoder is not configured");
    return false;
  }
  const std::filesystem::path directory =
      evidence_directory(workspace_directory);
  std::error_code error;
  std::filesystem::file_status status =
  ...
  if (error) {
    store_error(
        diagnostics, "cannot inspect evidence directory: " + error.message());
    return false;
  }
  if (status.type() == std::filesystem::file_type::not_found) {
    state = {};
  ...
  return load_state_unlocked(
      workspace_directory, key, codec, state, diagnostics);
}

EvidenceAttemptCommitResult commit_evidence_attempt(
    const std::filesystem::path &workspace_directory,
    const Sha256Digest &key,
    EvidenceAttemptCodec codec,
    DiagnosticSink &diagnostics) {
  EvidenceAttemptCommitResult result;
  if (codec.encode == nullptr || codec.decode == nullptr) {
    store_error(diagnostics, "typed evidence codec is not configured");
    return result;
  }
  const std::filesystem::path directory =
      evidence_directory(workspace_directory);
  ...
  EvidenceAttemptState current;
  if (!load_state_unlocked(
          workspace_directory, key, codec, current, diagnostics)) {
    return result;
  }
  ...
  const std::uint64_t ordinal =
      static_cast<std::uint64_t>(current.attempts.size()) + 1;

  std::string bytes;
  bool encoded_passed = false;
  if (!codec.encode(
          codec.state,
          key,
          ordinal,
  ...
  if (bytes.size() > kMaximumEvidenceBytes) {
    store_error(diagnostics, "compiler-produced evidence exceeds its size limit");
    return result;
  }
  bool decoded_passed = false;
  if (!codec.decode(
          codec.state,
          bytes,
          key,
  ...
  if (encoded_passed != decoded_passed) {
    store_error(
        diagnostics,
        "typed evidence codec disagrees about the attempt verdict");
    return result;
  }
  ...
  if (decoded_passed) next.active = digest;
  if (!write_atomic(
          state_path(workspace_directory, key),
          serialize_state(next),
          diagnostics)) {
    return result;
  }
